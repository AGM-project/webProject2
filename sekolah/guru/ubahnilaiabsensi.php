...
$nabsi = query("SELECT siswa.id id_siswa, siswa.nama nama, siswa.kelas kelas, siswa.nisn nisn, siswa.foto foto, siswa.alamat alamat,
  nilaiabsensi.id id_absensi, nilaiabsensi.nilai_absensi nilai_absensi, nilaiabsensi.status status
 FROM siswa LEFT JOIN nilaiabsensi ON siswa.id = nilaiabsensi.id_siswa WHERE siswa.id = '$id'")[0];
...
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Ubah Nilai Absensi Siswa</title>
...
<ul class="navbarul">
  <li class="navbarli"><img src="../img/vinama.jpg" width="50"></li>
	...
	<h1 class="judulubah">Ubah Nilai Absensi Siswa</h1>
	<br>

	<form action="" method="post" enctype="multipart/form-data">
		<input type="hidden" name="id" value="<?= $nabsi["id_siswa"];  ?>">
		<ul>
			<li>
				<label for="nama">Nama : </label>
				<input type="text" name="nama" id="nama" value="<?= $nabsi["nama"]; ?>" readonly>
			</li>
			
			<li>
				<label for="nilai_absensi">Nilai Absensi : </label><br>
				<input type="text" name="nilai_absensi" id="nilai_absensi" value="<?= $nabsi["nilai_absensi"] ?>">
			</li>
			<li>
